...
const AudioEngine = (() => {
  ...
  /* サムネイルのdata URLをBlob URLに変換してMediaSessionで使用 */
  function _getArtworkBlobUrl(thumbnail) {
    if (!thumbnail) return 'assets/icons/icon-512.png';
    if (!thumbnail.startsWith('data:')) return thumbnail;
    try {
      if (_artworkBlobUrl) { URL.revokeObjectURL(_artworkBlobUrl); _artworkBlobUrl = null; }
      const arr = thumbnail.split(',');
      const mime = (arr[0].match(/:(.*?);/) || [])[1] || 'image/jpeg';
      const bstr = atob(arr[1]);
      const u8arr = new Uint8Array(bstr.length);
      for (let i = 0; i < bstr.length; i++) u8arr[i] = bstr.charCodeAt(i);
      _artworkBlobUrl = URL.createObjectURL(new Blob([u8arr], { type: mime }));
      return _artworkBlobUrl;
    } catch (e) { return 'assets/icons/icon-512.png'; }
  }

  function _updateMediaSession() {
    if (!('mediaSession' in navigator)) return;
    const track = AppState.currentTrackId ? AppState.getTrack(AppState.currentTrackId) : null;
    const rawSrc = track?.thumbnail || null;
    const artworkSrc = rawSrc ? _getArtworkBlobUrl(rawSrc) : 'assets/icons/icon-512.png';
    const artworkType = rawSrc?.startsWith('data:image/jpeg') ? 'image/jpeg'
      : rawSrc?.startsWith('data:image/webp') ? 'image/webp'
      : artworkSrc.endsWith('.png') ? 'image/png' : 'image/jpeg';
    try {
    ...
    try { navigator.mediaSession.playbackState = AppState.isPlaying ? 'playing' : 'paused'; } catch {}
  }
