...
const PlayerScreen = (() => {
  ...
  const PlayerWidgetUI = {
    ...
    updateSpeed() {
      ...
      _updateSpeedButtons('pw-speed-btns', v);
      _updateSpeedButtons('po-speed-btns', v);
    },
  ...
  /* ── Fix: lucide replaces <i> with <svg>, handle both cases ── */
  function _setCtrlIcon(btnId, iconName) {
    const btn = el(btnId);
    if (!btn) return;
    // Try to find existing <i data-lucide> element
    let ico = btn.querySelector('i[data-lucide]');
    if (!ico) {
      // Lucide already replaced <i> with <svg> — recreate <i> element
  ...
  function _bindSpeedButtons(wrapId) {
  ...
  function _updateSpeedButtons(wrapId, value) {
    const wrap = el(wrapId);
    if (!wrap) return;
    wrap.querySelectorAll('.speed-btn').forEach(btn => {
  ...
  function _bindPlayerControls() {
    ...
    _bindSpeedButtons('pw-speed-btns');
    ...
    _bindSpeedButtons('po-speed-btns');
